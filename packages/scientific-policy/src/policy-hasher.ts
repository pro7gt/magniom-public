 ...
 * Deterministic JSON stringifier and SHA-256 hasher (Universal / Zero-dependency).
 * Conforms to IEC 62304 determinism invariants.
 */

/**
 * Deterministic JSON stringifier with key sorting
 */
...
/**
 * Standard SHA-256 implementation using typed arrays
 */
function sha256Universal(msg: string): string {
  ...
  // Append length in 64 bits big-endian
...
/**
 * Computes deterministic SHA-256 hash in hex format
 */
export function computeSha256(data: unknown): string {
  const canonical = typeof data === 'string' ? data : canonicalJsonStringify(data);
  return sha256Universal(canonical);
}
