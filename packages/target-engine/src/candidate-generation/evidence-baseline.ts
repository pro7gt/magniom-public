...
import { CANONICAL_MDD_TARGET_FAMILIES, type TargetFamilyDefinition } from './family-registry.js';
...
export function generateEvidenceBaselineCandidates(
  ...
  return families.map((family: TargetFamilyDefinition) => {
    ...
    if (family.id === 'TF-MDD-LDLPFC-EST-001' || family.code === 'LEFT_DLPFC_BA46') {
      candidateId = `${prefix}-evidence-ldlpfc`;
    } else if (family.id === 'TF-MDD-ANXIOSOMATIC-DMPFC-001' || family.code === 'DMPFC_BA9_32') {
