...
export function calculateCandidatePhenotypeConcordance(
  ...
  const isDmpfc =
    candidate.familyId?.includes('ANXIOSOMATIC') || candidate.circuitId?.includes('ANXIOSOMATIC');

  if (primaryDomain === 'DOMAIN-MDD-DYSPHORIC-001') {
    if (isDlpfc) {
      const score = primaryWeight + 0.1 * secondaryWeight;
    ...
    if (isDlpfc) {
      ...
      const score = secondaryWeight + 0.1 * primaryWeight;
      return Number(score.toFixed(2));
    }
  }
...
export function attachPhenotypeConcordance(
  ...
  return candidates.map(candidate => {
    const score =
      candidate.phenotypeConcordanceScore > 0 &&
      candidate.phenotypeConcordanceScore !== candidate.evidenceScore
