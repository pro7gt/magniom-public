...
function verifySprint4() {
  console.log('🚀 Running Sprint 4 (Evidence Knowledge Graph & Evidence Ceiling) Verification...\n');
  ...
  console.log('   ✓ Invariant checks passed: immutability triggers, graph traversal RPCs, active release export.');
  ...
  console.log('   ✓ pgTAP test plan verified (28 tests for tables, views, functions, RLS, and seed counts).');
  ...
  console.log(`   ✓ Claims: ${validatedRelease.claims.length}, Circuits: ${validatedRelease.circuits.length}, Target Families: ${validatedRelease.families.length}`);
  ...
  console.log(`   ✓ Graph Traversal: Resolved ${paths.length} multi-hop evidence path(s) for TF-MDD-CONVERGENT-LDLPFC-001.`);
  ...
  console.log(`   ✓ Negative Evidence: Attached negative constraint claim ${superiorityConflict.claimCode} (${superiorityConflict.sourceCitation}).`);
  ...
  if (clinicalSlate.primaryCandidates.some((c) => c.familyId === 'TF-RES-CING-L8AV-001')) {
    console.error('❌ Clinical Mode violation: Tier 4 research candidate was admitted to clinical primary slate.');
    process.exit(1);
  }
  console.log('   ✓ Clinical Mode Ceiling Enforced: Tier 4 research candidate (Cingulum L8Av) was disqualified from Clinical Mode.');
  ...
  const researchCand = researchSlate.additionalCandidates.find((c) => c.familyId === 'TF-RES-CING-L8AV-001');
  if (!researchCand || researchCand.evidenceTier !== 'T4') {
    console.error('❌ Research Mode failure: Tier 4 candidate was not properly admitted in Research Mode.');
    process.exit(1);
  }
  console.log('   ✓ Research Mode: Tier 4 research target permitted and allocated to Additional role.');
  ...
  for (let i = 0; i < 100; i++) {
    ...
    if (run.deterministicManifestHash !== firstHash) {
      console.error(`❌ Non-deterministic hash detected at iteration ${i}: ${run.deterministicManifestHash} vs ${firstHash}`);
      process.exit(1);
    }
  }
