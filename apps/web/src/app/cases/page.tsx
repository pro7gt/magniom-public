...
export default function CasesRegistryPage() {
  const [filter, setFilter] = useState<'all' | 'mdd' | 'awaiting_review' | 'stale' | 'research'>('all');
  ...
  const filteredCases = allCases.filter((c) => {
    ...
    if (filter === 'awaiting_review') return c.state === 'target_slate_ready' || c.state === 'phenotype_ready';
    ...
    <div className="container" style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '1rem' }}>
        ...
        <div style={{ display: 'flex', gap: '0.75rem' }}>
          <Link href="/validation" className="btn btn-secondary" title="Open Validation and Golden Cases harness">
            🧪 Golden Cases Suite
          </Link>
          <button
            onClick={() => {
