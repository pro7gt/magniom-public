...
export default function CaseOverviewPage({
  ...
  const slateVM = toTargetSlateViewModel(record.slate, { isStale: record.isStale, staleReason: record.staleReason });

  return (
    <div className="container" style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
      {/* 10-Second Orientation Banner */}
      <div className="card" style={{ background: 'linear-gradient(135deg, #0f172a 0%, #1e293b 100%)', borderColor: '#334155' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '1rem' }}>
          <div>
            ...
            <h1 style={{ fontSize: '1.5rem', fontWeight: 700, color: '#fff', marginTop: '0.25rem' }}>
              {phenotypeVM.primaryDiagnosis}
            </h1>
            <p style={{ color: '#cbd5e1', fontSize: '0.875rem', marginTop: '0.25rem' }}>
              Case Code: <strong style={{ fontFamily: 'var(--font-mono)' }}>{record.clinicalCase.caseCode}</strong> • Mode: <strong>{record.clinicalCase.mode}</strong>
            </p>
          </div>
          ...
          <div style={{ display: 'flex', gap: '0.75rem' }}>
            ...
            <Link href={`/cases/${caseId}/targets`} className="btn btn-primary" id="review-target-slate-overview-btn">
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(320px, 1fr))', gap: '1.5rem' }}>
        ...
        <div className="card">
          ...
          <h2 style={{ fontSize: '1.125rem', fontWeight: 700, marginTop: '0.25rem', marginBottom: '0.75rem' }}>
            What are we trying to improve?
          </h2>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem', fontSize: '0.875rem' }}>
            {phenotypeVM.domains.slice(0, 3).map((d) => (
              <div key={d.id} style={{ display: 'flex', justifyContent: 'space-between', background: 'var(--bg-surface-elevated)', padding: '0.5rem 0.75rem', borderRadius: '0.375rem' }}>
                <span>#{d.clinicalPriority} {d.domainName}</span>
        ...
        <div className="card">
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem', fontSize: '0.875rem' }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between' }}>
              ...
              <span><strong>27.4</strong> usable minutes</span>
            </div>
            <div style={{ display: 'flex', justifyContent: 'space-between' }}>
        ...
        <div className="card">
          ...
          <p style={{ fontSize: '0.8125rem', color: '#cbd5e1', marginBottom: '0.75rem' }}>
            Magniom identified <strong>{slateVM.primaryCandidates.length} primary hypotheses</strong> and <strong>{slateVM.additionalCandidates.length} alternatives</strong> based on approved phenotype and evidence ceilings.
          </p>
          <div style={{ background: '#090d16', padding: '0.5rem 0.75rem', borderRadius: '0.375rem', fontSize: '0.75rem', color: 'var(--text-muted)' }}>
        ...
        <div className="card">
          ...
          <Link href={`/cases/${caseId}/decision`} className="btn btn-secondary" style={{ width: '100%', fontSize: '0.8125rem' }}>
            {record.decision?.isImmutable ? 'View Signed Decision Record →' : 'Begin Clinical Decision →'}
          </Link>
        </div>
      </div>
    </div>
