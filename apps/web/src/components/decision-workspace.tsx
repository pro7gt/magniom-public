...
export function DecisionWorkspace({
  ...
  const [candidateActions, setCandidateActions] = useState<Record<string, CandidateActionState>>(() => {
    ...
    slateVM.primaryCandidates.forEach((c) => {
      initial[c.id] = { action: 'accept' as CandidateDecisionAction, reasons: [ACCEPT_REASONS[0] || 'Strong clinical phenotype fit'] };
    });
    return initial;
  });
  ...
  const [disagreement, setDisagreement] = useState(existingDecisionVM?.disagreementWithMagniom || '');
  ...
  const handleActionChange = (candidateId: string, action: CandidateDecisionAction) => {
    ...
    const defaultReasons = action === 'accept' ? [ACCEPT_REASONS[0] || 'Strong clinical phenotype fit'] : [REJECT_REASONS[0] || 'Patient preference / prior response'];
  ...
  return (
    <div className="container" style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: '1rem' }}>
        <div>
          ...
          <p style={{ color: 'var(--text-secondary)', fontSize: '0.875rem' }}>
            Authoritative clinical decision-making workspace. Final targeting selection requires explicit clinician review and independent rationale.
          </p>
        </div>
      ...
      {isImmutable && (
        <div style={{ background: 'rgba(16, 185, 129, 0.15)', border: '1px solid #059669', borderRadius: '0.5rem', padding: '1rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div>
            ...
            <p style={{ color: '#e2e8f0', fontSize: '0.8125rem', marginTop: '0.25rem' }}>
              Signed by <strong>{existingDecisionVM.clinicianName}</strong> ({existingDecisionVM.clinicianLicense}) on {existingDecisionVM.signedAt}
            </p>
          ...
          {onCreateRevisedDecision && (
            <button onClick={onCreateRevisedDecision} className="btn btn-secondary" style={{ borderColor: 'var(--accent-cyan)' }}>
      ...
      <div className="decision-grid">
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
          <div className="card">
            ...
            <p style={{ fontSize: '0.8125rem', color: 'var(--text-secondary)', marginBottom: '1rem' }}>
              For each candidate on the Target Slate, indicate whether you accept, reject, modify, or defer targeting.
            </p>
                ...
                return (
                  ...
                  >
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                      <div>
                        <strong style={{ color: 'var(--accent-cyan)' }}>{candidate.roleTitle}</strong>
                    ...
                    {!isImmutable && (
                      <div style={{ background: 'rgba(9, 13, 22, 0.6)', padding: '0.75rem', borderRadius: '0.375rem', fontSize: '0.8125rem' }}>
                    ...
                    {isModify && (
                      <div style={{ background: '#0b1622', border: '1px solid #1e3a5f', padding: '0.75rem', borderRadius: '0.375rem', fontSize: '0.8125rem' }}>
                        <strong style={{ color: '#38bdf8' }}>Clinician Coordinate Adjustment (MNI152):</strong>
                        ...
                        <p style={{ color: 'var(--text-secondary)', fontSize: '0.75rem', marginTop: '0.375rem' }}>
                          Live Euclidean delta from original candidate will be recorded immutably in the signed record.
                        </p>
                      </div>
                    )}
                  </div>
          ...
          <div className="card">
            ...
            <p style={{ fontSize: '0.8125rem', color: 'var(--text-secondary)', marginBottom: '0.75rem' }}>
              Summarise why the selected target(s) best address the current clinical formulation and treatment objectives.
            </p>
            <textarea
          ...
          {isPrimary1Rejected && (
            <div className="card" style={{ background: '#1c1214', borderColor: '#7f1d1d' }}>
              <h3 style={{ fontSize: '0.9375rem', fontWeight: 600, color: '#f87171', marginBottom: '0.5rem' }}>
                Scientific Disagreement / Override Formulation
              </h3>
              <p style={{ fontSize: '0.8125rem', color: '#fca5a5', marginBottom: '0.5rem' }}>
                You have rejected Primary Candidate 1. Please document the specific points of divergence from Magniom's algorithmic nomination:
              </p>
              <textarea
                className="form-textarea"
                value={disagreement}
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
          ...
          <div className="card">
            ...
            <p style={{ fontSize: '0.8125rem', color: 'var(--text-secondary)', marginBottom: '0.75rem' }}>
              How much did Magniom alter your target decision compared to prior independent reasoning?
            </p>
          ...
          <div className="card">
            ...
            <div className="form-group">
              <label className="form-label" style={{ fontSize: '0.75rem' }}>Clinician Full Name:</label>
              <input
                type="text"
                className="form-input"
                value={clinicianName}
            ...
            <div className="form-group">
              <label className="form-label" style={{ fontSize: '0.75rem' }}>Professional Registration / License:</label>
              <input
                type="text"
                className="form-input"
                value={licenseNumber}
          ...
          {!isImmutable && (
            <div className="card" style={{ background: '#0a1322', borderColor: '#1e3a5f' }}>
              <h3 style={{ fontSize: '0.9375rem', fontWeight: 700, color: 'var(--accent-cyan)', marginBottom: '0.5rem' }}>
                Legal & Clinical Attestation
              </h3>
              <p style={{ fontSize: '0.8125rem', color: '#cbd5e1', marginBottom: '0.75rem', lineHeight: 1.4 }}>
                {attestationStatement}
              </p>

              <label style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', cursor: 'pointer', marginBottom: '1rem', fontSize: '0.875rem' }}>
                <input
                  type="checkbox"
                ...
                />
                <strong style={{ color: '#fff' }}>I independently confirm and sign this decision</strong>
              </label>

              <button
                onClick={handleExecuteSign}
                disabled={!attestationConfirmed || !overallReasoning.trim() || isSigning || slateVM.isStale}
                className="btn btn-primary"
                style={{ width: '100%', padding: '0.75rem', fontSize: '0.9375rem', fontWeight: 700 }}
