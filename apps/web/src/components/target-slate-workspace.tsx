...
export function TargetSlateWorkspace({
  ...
  const clinical3dVM = useMemo(() => {
    return toClinical3DViewerViewModel(
      fallbackSlate,
      rawDomainCandidates,
      selectedCandidateId
    );
    ...
    <div className="workspace-3col">
      ...
      <aside className="col-context" aria-label="Clinical Context Column">
        <div className="card">
          ...
          <h2 style={{ fontSize: '1.125rem', fontWeight: 700, color: 'var(--text-primary)', marginTop: '0.25rem' }}>
            {phenotypeVM.primaryDiagnosis}
          </h2>
          <span className="badge badge-tier1" style={{ marginTop: '0.5rem', alignSelf: 'flex-start' }}>
        ...
        <div className="card">
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem', fontSize: '0.8125rem' }}>
            {phenotypeVM.domains.slice(0, 3).map((domain) => (
              ...
              >
                <div>
                  <strong>#{domain.clinicalPriority} {domain.domainName}</strong>
                </div>
                <span style={{ color: domain.severityLabel === 'Severe' || domain.severityLabel === 'Extreme' ? '#f87171' : '#38bdf8', fontWeight: 600, fontSize: '0.75rem' }}>
        ...
        <div className="card">
          ...
          <ul style={{ listStyle: 'none', display: 'flex', flexDirection: 'column', gap: '0.5rem', fontSize: '0.8125rem', color: '#cbd5e1' }}>
        ...
        <div className="card" style={{ background: '#0d1627' }}>
          <h3 style={{ fontSize: '0.9375rem', fontWeight: 600, color: 'var(--accent-cyan)', marginBottom: '0.5rem' }}>
            Personalisation Status
          </h3>
          <p style={{ fontSize: '0.8125rem', color: '#cbd5e1' }}>
            {slateVM.qualificationLabel}
          </p>
      ...
      <section className="col-spatial" aria-label="Spatial and Convergence Column">
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.5rem' }}>
          <div>
            ...
            <p style={{ fontSize: '0.8125rem', color: 'var(--text-secondary)' }}>
              Interactive cortical surface inspection, target ROI localization, and evidence counterfactual analysis.
            </p>
          </div>

          <div style={{ display: 'flex', gap: '0.5rem' }}>
            <Link href={`/cases/${caseId}/compare`} className="btn btn-secondary" style={{ fontSize: '0.8125rem' }}>
              Compare Matrix →
            </Link>
            <Link href={`/cases/${caseId}/decision`} className="btn btn-primary" style={{ fontSize: '0.8125rem' }} id="begin-decision-btn">
        ...
        <div className="card" style={{ marginTop: '1rem' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.5rem' }}>
            <h3 style={{ fontSize: '0.9375rem', fontWeight: 600 }}>Target Convergence Diagnostic</h3>
            <span className={`badge ${convergenceVM.badgeClass}`}>
              {convergenceVM.headline}
            </span>
          </div>
          ...
          {convergenceVM.details.length > 0 && (
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: '0.5rem' }}>
              {convergenceVM.details.map((item, idx) => (
                <div key={idx} style={{ background: 'var(--bg-surface-elevated)', padding: '0.5rem', borderRadius: '0.375rem', fontSize: '0.75rem' }}>
                  <span style={{ color: 'var(--text-secondary)' }}>{item.sourceName}:</span>
                  <div style={{ fontFamily: 'var(--font-mono)', fontWeight: 600, color: 'var(--text-primary)', marginTop: '0.15rem' }}>
      ...
      <section className="col-slate" aria-label="Target Slate Candidates Column">
        <div>
          ...
          <p style={{ fontSize: '0.8125rem', color: 'var(--text-secondary)' }}>
            {slateVM.primaryCandidates.length} Primary Hypotheses + {slateVM.additionalCandidates.length} Alternatives
          </p>
        </div>
            ...
            <div className="card" style={{ background: '#11141e', borderStyle: 'dashed' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                {slateVM.suppressedCandidates.map((suppressed) => (
                  <div key={suppressed.id} style={{ fontSize: '0.75rem', color: 'var(--text-muted)' }}>
                    <strong style={{ color: '#9ca3af' }}>{suppressed.name}</strong>: {suppressed.explanation}
                  </div>
                ))}
              </div>
            </div>
