...
export function EvidenceDrawer({ isOpen, onClose, viewModel }: EvidenceDrawerProps) {
  ...
  return (
    <div className="drawer-backdrop" onClick={onClose} role="dialog" aria-modal="true" aria-label="Evidence Drawer">
      <div className="drawer-panel" onClick={(e) => e.stopPropagation()}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', borderBottom: '1px solid var(--border-color)', paddingBottom: '1rem' }}>
          ...
          <button onClick={onClose} className="btn btn-secondary" style={{ padding: '0.25rem 0.625rem' }} aria-label="Close Drawer">
            ✕
          </button>
        </div>

        {/* Clinical Claim & Evidence Tier */}
        <div style={{ background: 'var(--bg-surface-elevated)', border: '1px solid var(--border-color)', borderRadius: '0.5rem', padding: '1rem', display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
        ...
        <div>
          <h3 style={{ fontSize: '0.875rem', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.05em', color: 'var(--text-secondary)', marginBottom: '0.75rem' }}>
          ...
          <div className="evidence-path-tree">
            {viewModel.evidencePath.map((node, idx) => (
              <div key={idx} className="evidence-path-node">
                ...
                <div style={{ fontSize: '0.75rem', color: 'var(--text-secondary)' }}>{node.description}</div>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="card" style={{ background: '#0b1622', borderColor: '#1e3a5f' }}>
          <h3 style={{ fontSize: '0.875rem', fontWeight: 700, color: '#38bdf8', marginBottom: '0.5rem', display: 'flex', alignItems: 'center', gap: '0.375rem' }}>
        ...
        <div className="card" style={{ background: '#1c1014', borderColor: '#5c1d24' }}>
          <h3 style={{ fontSize: '0.875rem', fontWeight: 700, color: '#f87171', marginBottom: '0.5rem', display: 'flex', alignItems: 'center', gap: '0.375rem' }}>
            ...
            {viewModel.studySources.map((study) => (
              ...
              >
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                  <span style={{ fontWeight: 600, fontSize: '0.8125rem', color: 'var(--accent-cyan)' }}>
                    {study.citation}
                  </span>
                  <button
          ...
          <div className="modal-backdrop" onClick={() => setSelectedStudy(null)}>
            <div className="modal-dialog" onClick={(e) => e.stopPropagation()} style={{ maxWidth: '680px' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', borderBottom: '1px solid var(--border-color)', paddingBottom: '0.75rem' }}>
                ...
                <button onClick={() => setSelectedStudy(null)} className="btn btn-secondary" style={{ padding: '0.2rem 0.5rem' }}>✕</button>
              </div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.875rem', fontSize: '0.8125rem' }}>
                <div>
                  ...
                  <p style={{ color: '#e2e8f0', marginTop: '0.15rem' }}>{selectedStudy.clinicalQuestion}</p>
                </div>

                <div>
                  <strong style={{ color: 'var(--accent-cyan)' }}>Study Population & Size:</strong>
                  <p style={{ color: '#e2e8f0', marginTop: '0.15rem' }}>{selectedStudy.studyPopulation}</p>
                </div>

                <div>
                  <strong style={{ color: 'var(--accent-cyan)' }}>Targeting & Protocol:</strong>
                  <p style={{ color: '#e2e8f0', marginTop: '0.15rem' }}>
                    Method: {selectedStudy.targetingMethod} | Protocol: {selectedStudy.protocolDelivered}
                  </p>
                </div>

                <div>
                  <strong style={{ color: 'var(--accent-cyan)' }}>Observed Clinical Outcome:</strong>
                  <p style={{ color: '#e2e8f0', marginTop: '0.15rem' }}>{selectedStudy.clinicalOutcome}</p>
                </div>

                <div style={{ background: '#090d16', padding: '0.625rem', borderRadius: '0.375rem', border: '1px solid #1e293b' }}>
                  <strong style={{ color: '#34d399' }}>Why Magniom Uses It:</strong>
                  <p style={{ color: '#e2e8f0', marginTop: '0.15rem' }}>{selectedStudy.whyMagniomUsesIt}</p>
                </div>

                <div style={{ background: '#1c1014', padding: '0.625rem', borderRadius: '0.375rem', border: '1px solid #5c1d24' }}>
                  <strong style={{ color: '#f87171' }}>What It Does Not Prove:</strong>
                  <p style={{ color: '#fca5a5', marginTop: '0.15rem' }}>{selectedStudy.whatItDoesNotProve}</p>
                </div>
              </div>
