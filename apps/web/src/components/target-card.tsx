...
export function TargetCard({
  ...
  return (
    ...
    >
      <div className="card-header-row">
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.25rem' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', flexWrap: 'wrap' }}>
            <span className={`badge ${candidate.isPrimary ? 'badge-role-primary' : 'badge-role-additional'}`}>
              {candidate.roleTitle}
            </span>
          ...
          <h3 style={{ fontSize: '1rem', fontWeight: 700, color: 'var(--text-primary)', marginTop: '0.25rem' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: 'rgba(9, 13, 22, 0.6)', padding: '0.5rem 0.75rem', borderRadius: '0.375rem' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
          <span style={{ color: 'var(--text-secondary)', fontSize: '0.75rem' }}>MNI:</span>
          <strong style={{ fontFamily: 'var(--font-mono)', color: 'var(--accent-cyan)', fontSize: '0.875rem' }}>
      ...
      {candidate.whatMriChanged.hasPersonalisation && candidate.counterfactualTarget && (
        <div className="counterfactual-box">
          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '0.25rem' }}>
            ...
            <strong style={{ fontFamily: 'var(--font-mono)' }}>{candidate.counterfactualTarget.coordinateFormatted}</strong>
          </div>
      ...
      {isExpanded && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem', borderTop: '1px solid var(--border-subtle)', paddingTop: '0.75rem', fontSize: '0.8125rem' }}>
          <div>
          ...
          <div>
            ...
            <p style={{ color: 'var(--text-primary)' }}>
              Depth: {candidate.anatomicalAccessibility.depthMm} mm | Distance to skull: {candidate.anatomicalAccessibility.skullDistanceMm} mm ({candidate.anatomicalAccessibility.rating})
            </p>
          </div>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderTop: '1px solid var(--border-subtle)', paddingTop: '0.5rem', marginTop: '0.25rem' }}>
        ...
        <button
          ...
          className="btn btn-secondary"
          style={{ padding: '0.25rem 0.625rem', fontSize: '0.75rem', color: 'var(--accent-cyan)', borderColor: 'rgba(56, 189, 248, 0.4)' }}
